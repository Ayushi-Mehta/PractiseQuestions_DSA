...
public class sir_reply {

	public static void main(String[] args) {
		queenComb2DKill(new boolean[5][5], 0, 0, 0, 5, "");
	}
	public static void queenComb2DKill(boolean[][] board, int row, int col, int qpsf, int tq, String ans) {
		...
		if (isItSafeToPlaceQueen(board, row, col)) {
			...
			queenComb2DKill(board, row, col + 1, qpsf + 1, tq, ans + "{r" + row + "c" + col + "}");
			board[row][col] = false;// queen removed from there
			// }
		}
		// queen not placed
		queenComb2DKill(board, row, col + 1, qpsf, tq, ans);
	}
